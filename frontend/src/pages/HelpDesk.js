...
function HelpDesk() {
  ...
  const {toast, showToast} = useToast(); // To get the toast instance
  ...
  const fetchTickets = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/admin/help-desk/tickets');
  ...
  const handleReply = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`http://localhost:5000/api/admin/help-desk/tickets/${selectedTicket._id}/reply`, { reply });
      setReply('');
  ...
  const handleClose = () => {
    setSelectedTicket(null); // To close the reply section
    setReply('');
  };
